...
class TransferBankLainViewC: UIViewController,UITextFieldDelegate
{
    ...
    var selectedBank : NSDictionary!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        
        self.title = "Transfer - Bank Lain"
        ...
        let fieldNamesArray : [NSString] = ["Bank Tujuan","Nomor Rekening Tujuan","Jumlah","mPIN"]
        ...
        for i in 1...fieldNamesArray.count {
            ...
            if( i == 1)
            {
                step1TextField.text = self.selectedBank.valueForKey("name") as? String
                step1TextField.enabled = false
                step1TextField.backgroundColor = UIColor(netHex: 0xDBDBDB)
            }else{
        ...
        step1FormAcceptBtn.addTarget(self, action: #selector(TransferBankLainViewC.cashInAcceptClicked(_:)), forControlEvents: UIControlEvents.TouchUpInside)
        ...
        let tap: UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(TransferBankLainViewC.dismissKeyboard))
    ...
    @IBAction func cashInAcceptClicked(sender: UIButton)
    {
        ...
        for  subView in contentView.subviews
        {
            if (subView.isKindOfClass(UITextField))
            {
                ...
                if(currentTextField.tag == 12  || currentTextField.tag == 13 || currentTextField.tag == 14)
                {
    ...
    func inqueryServiceRequest ()
    {
        ...
        dict[DESTACCOUNTNUMBER] = cashInFormDictonary["mobileNumber"] as! NSString
        
        
        // -------------- Bank Lain/ Other Banks -------------- ///

        dict[DEST_BANK_CODE] = self.selectedBank.valueForKey("code") as! NSString
            ...
            dict, successBlock: { (response) -> Void in
                ...
                dispatch_async(dispatch_get_main_queue()) {
                    ...
                    if( messagecode == SIMOBI_SELFBANK_TRANSFER_INQ_SUCCESSCODE)
                    {
                        ...
                        dict[TXNNAME] = TXN_CONFIRM_OTHERBANK
                        dict[SCTL_ID] = sctlID
                        
                        let confirmationTitlesArray = NSMutableArray()
                        let confirmationValuesArray = NSMutableArray()
                        
                        let accountName = responseDict.valueForKeyPath("response.destinationName.text") as! String
                        let destinationNumber = self.cashInFormDictonary["mobileNumber"] as! NSString
                        
                        let amount = self.cashInFormDictonary["amount"]  as! NSString
                        ...
                        confirmationValuesArray[1] = self.selectedBank.valueForKey("name") as! String
                        ...
                        if(amount.length > 0)
                        {
                            confirmationTitlesArray[3] = "Jumlah"
                            confirmationValuesArray[3] = "Rp \(amount)"
                        }
                        
                        if(charges.length > 0)
                        {
                            confirmationTitlesArray[4] = "Biaya Transfer"
                            confirmationValuesArray[4] = "Rp \(charges)"
                        }
                        
                        if(totalDebitAmount.length > 0)
                        {
                            confirmationTitlesArray[5] = "Total Pendebitan"
                            confirmationValuesArray[5] = "Rp \(totalDebitAmount)"
                        }
                        ...
                        confirmationViewController.simasPayOptionType = SimasPayOptionType.SIMASPAY_TRANSFER_BANK_LAIN
