...
class TransactionHistoryViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,QLPreviewControllerDataSource,QLPreviewControllerDelegate
{
    ...
    override func viewDidLoad() {
        ...
        if(isMoreDataAvailable == true)
        {
            ...
            loadControl.addTarget(self, action: #selector(TransactionHistoryViewController.loadMoreTransactionsData(_:)), forControlEvents: UIControlEvents.ValueChanged)
        ...
        if (self.simasPayOptionType == SimasPayOptionType.SIMASPAY_REGULAR_TRANSAKSI)
        {
            ...
            titleLabel.text = "Periode: \(periodString)"
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if (self.simasPayOptionType != SimasPayOptionType.SIMASPAY_REGULAR_TRANSAKSI)
        {
            ...
            downLoadButton.frame = CGRectMake(0, 0, 36, 36)
            downLoadButton.addTarget(self, action: #selector(TransactionHistoryViewController.downLoadButtonClicked(_:)), forControlEvents: UIControlEvents.TouchUpInside)
            downLoadButton.setImage(UIImage(named: "download"), forState: UIControlState.Normal)
            
            let downLoadButtonItem = UIBarButtonItem(customView: downLoadButton)
            ...
            dict, successBlock: { (response) -> Void in
                ...
                dispatch_async(dispatch_get_main_queue()) {
                    ...
                    if( messagecode == SIMAPAY_TRANSACTION_HISTORY_CODE)
                    {
                        ...
                        {
                            for i in 0  ..< newTransactionArray.count 
                            {
                    ...
                    if( messagecode == DownloadTransactionHistory_Code)
                    {
                        ServiceModel.downloadDataForURL(downLoadPDFURL,  successBlock: { (response) -> Void in
                            ...
                            dispatch_async(dispatch_get_main_queue()) {
                                ...
                                let fileURL = documentsURL.URLByAppendingPathComponent("Mutasi.pdf")
