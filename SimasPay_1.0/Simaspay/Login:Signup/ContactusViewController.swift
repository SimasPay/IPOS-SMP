...
class ContactusViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate,MFMailComposeViewControllerDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell{
        ...
        if indexPath.row == 0
        {
            ...
            let tapGestureRecognizer = UITapGestureRecognizer(target:self, action:#selector(ContactusViewController.image1Tapped(_:)))
        ...
        {
            imageView.image = UIImage(named:"ic-phone")
            let tapGestureRecognizer = UITapGestureRecognizer(target:self, action:#selector(ContactusViewController.image2Tapped(_:)))
            imageView.userInteractionEnabled = true
            imageView.addGestureRecognizer(tapGestureRecognizer)
